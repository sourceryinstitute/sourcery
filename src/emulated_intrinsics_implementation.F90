...
contains
  ...
  module procedure co_sum_real32_1D_array
    ...
    real(real32), allocatable :: total(:)[:]
    integer, parameter :: root_node=1, default_root_image=1
    integer even_child_image, odd_child_image, parent_image, my_image
    integer even_child_node , odd_child_node , parent_node , my_node, default_parent_image
    integer, allocatable :: relatives(:)

    allocate(total(size(a))[*])
  ...
  module procedure co_sum_real32_2D_array
    ...
    allocate(total(size(a,1),size(a,2))[*])
  ...
  module procedure co_broadcast_real32_1D_array
    ...
    real(real32), allocatable :: message(:)[:]
    integer, parameter :: root_node=1, default_root_image=1
    integer even_child_image, odd_child_image, parent_image, my_image
    integer even_child_node , odd_child_node , parent_node , my_node, default_parent_image
    integer, allocatable :: relatives(:)
  ...
  module procedure co_broadcast_real32_2D_array
    ...
    allocate(message(size(a,1), size(a,2))[*])
  ...
  module procedure co_broadcast_real64_1D_array
    ...
    real(real64), allocatable :: message(:)[:]
    integer, parameter :: root_node=1, default_root_image=1
    integer even_child_image, odd_child_image, parent_image, my_image
    integer even_child_node , odd_child_node , parent_node , my_node, default_parent_image
    integer, allocatable :: relatives(:)
  ...
  module subroutine co_reduce_logical(a,operation,result_image,stat,errmsg)
    ...
    !! Binary tree collective reduction

    logical, save :: reduction[*]
    integer, parameter :: root_node=1, default_root_image=1
    integer even_child_image, odd_child_image, parent_image, my_image
    integer even_child_node , odd_child_node , parent_node , my_node, default_parent_image
    integer, allocatable :: relatives(:)
    ...
    if ( exists(even_child_node) ) then
      sync images(even_child_image)
      reduction = operation(reduction, reduction[even_child_image])
    end if
    ...
    if ( exists(odd_child_node) )  then
      sync images(odd_child_image)
      reduction = operation(reduction, reduction[odd_child_image])
    end if

    if (exists(parent_image)) sync images(parent_image)

    if (present(result_image)) then
      if (my_image==result_image) a = reduction
    else
      call co_broadcast(reduction,source_image=default_root_image)
      a = reduction
    end if
